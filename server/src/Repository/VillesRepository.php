...
class VillesRepository extends ServiceEntityRepository
{
    ...
    public function saveVille($data){
        $newVille = new Villes();
        $newVille->setNomVille($data['nomVille']);
        $this->manager->persist($newVille);
        $this->manager->flush();
    }

    public function removeVille(Villes $ville){
        $this->manager->remove($ville);
        $this->manager->flush();
    }
    
    public function updateVille(Villes $ville): Villes {
